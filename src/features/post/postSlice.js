...
export const editPost =
  ({ editPostId, content, image, userId, page }) =>
  async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      let imageUrl;
      if (image) imageUrl = await cloudinaryUpload(image);
      const response = await apiService.put(`/posts/${editPostId}`, {
        content,
        image: imageUrl,
      });
